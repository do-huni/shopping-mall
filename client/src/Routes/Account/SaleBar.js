...
function SaleBar(props) {
  ...
  return (
	  <>
	  	<div style = {{width: "15%"}}>
				<div className = "bg-dark col-auto d-flex justify-content-between flex-column px-1">
					<div>
						<a className = "text-decoration-none text-white d-none d-sm-inline d-flex align-items-center ms-3 mt-3">
							<span className = "ms-1 fs-4 d-none d-sm-inline">{props.title}</span>
						</a>
							...
							{props.items.map((data, idx)=>{
								return (
									<li key = {idx} className = "nav-item text-white fs-4 my-1 py-1 py-sm-0">
										<a onClick = {()=>navigate("/test")} className = "nav-link text-white fs-5" aria-current = "page">
